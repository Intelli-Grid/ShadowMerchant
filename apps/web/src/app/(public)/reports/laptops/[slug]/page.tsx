...
export async function generateMetadata({ params }: ReportPageProps): Promise<Metadata> {
  ...
  const report = laptopData.find((r) => r.slug === slug);
  ...
  return {
    ...
    openGraph: {
      ...
      url: `https://www.shadowmerchant.online/reports/laptops/${slug}`,
    },
    alternates: { canonical: `https://www.shadowmerchant.online/reports/laptops/${slug}` },
  };
}

export async function generateStaticParams() {
  return laptopData.map((r) => ({ slug: r.slug }));
}

export default async function LaptopReportDetailPage({ params }: ReportPageProps) {
  const { slug } = await params;
  const report = laptopData.find((r) => r.slug === slug);
  if (!report) notFound();
  ...
  return (
    <main className="w-full max-w-4xl mx-auto px-4 py-10 sm:py-14">
      <Link href="/reports/laptops" className="text-xs font-semibold text-gold hover:underline mb-6 inline-block">
        ← Back to all Laptop Decision Reports
      </Link>

      <article className="prose prose-invert max-w-none">
        <div className="flex items-center gap-3 mb-3">
          <span className="text-xs font-semibold uppercase tracking-wider text-gold">
            💻 Laptop Decision Report #{report.id}
          </span>
          <span
        ...
        <div className="mt-12 pt-6 border-t text-xs text-gray-400 space-y-3" style={{ borderColor: 'var(--sm-border)' }}>
          ...
          <div className="pt-3 flex gap-4 text-gold font-semibold flex-wrap">
            <Link href="/guides/laptop-variant-guide" className="hover:underline">Read: Laptop Variant Comparison Guide →</Link>
            <Link href="/guides/strikethrough-mrp-myth" className="hover:underline">Read: Strikethrough MRP Myth →</Link>
          </div>
        </div>
      </article>
    </main>

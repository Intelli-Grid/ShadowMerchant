...
export async function generateMetadata({ params }: ReportPageProps): Promise<Metadata> {
  ...
  const report = applianceData.find((r) => r.slug === slug);
  ...
  return {
    ...
    openGraph: {
      ...
      url: `https://www.shadowmerchant.online/reports/appliances/${slug}`,
    },
    alternates: { canonical: `https://www.shadowmerchant.online/reports/appliances/${slug}` },
  };
}

export async function generateStaticParams() {
  return applianceData.map((r) => ({ slug: r.slug }));
}

export default async function ApplianceReportDetailPage({ params }: ReportPageProps) {
  const { slug } = await params;
  const report = applianceData.find((r) => r.slug === slug);
  if (!report) notFound();
  ...
  return (
    <main className="w-full max-w-4xl mx-auto px-4 py-10 sm:py-14">
      <Link href="/reports/appliances" className="text-xs font-semibold text-gold hover:underline mb-6 inline-block">
        ← Back to all Appliance Decision Reports
      </Link>

      <article className="prose prose-invert max-w-none">
        <div className="flex items-center gap-3 mb-3">
          <span className="text-xs font-semibold uppercase tracking-wider text-gold">
            🏠 Appliance Decision Report #{report.id}
          </span>
          <span

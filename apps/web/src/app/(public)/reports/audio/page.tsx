...
export const metadata: Metadata = {
  title: 'Audio & Headphones Decision Reports (2026) | ShadowMerchant',
  description: 'Evidence-backed audio buying decision reports in India. Observed 30-day median prices, exact model matching, and honest Buy/Wait recommendations.',
  openGraph: {
    title: 'Audio & Headphones Decision Reports | ShadowMerchant',
    description: 'Independent audio decision reports backed by observed daily price tracking.',
    url: 'https://www.shadowmerchant.online/reports/audio',
  },
  alternates: { canonical: 'https://www.shadowmerchant.online/reports/audio' },
};

const formatPrice = (price: number) =>
  new Intl.NumberFormat('en-IN', { style: 'currency', currency: 'INR', maximumFractionDigits: 0 }).format(price);

export default function AudioReportsIndexPage() {
  return (
    <main className="w-full max-w-6xl mx-auto px-4 py-10 sm:py-14">
      <div className="text-center mb-12">
        ...
        >
          🎧 Vertical Expansion — Audio, Headphones & TWS
        </span>
        <h1 className="text-3xl sm:text-4xl font-extrabold mt-4 text-white tracking-tight">
          Audio & Headphones Buying Decision Reports
        </h1>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        {audioData.map((audio) => {
          const isBuy = audio.recommendation === 'BUY';
          return (
            <Link
              key={audio.slug}
              href={`/reports/audio/${audio.slug}`}
            ...
            >
              <div>
                <div className="flex items-center justify-between gap-2 text-xs font-medium mb-3">
                  <span className="text-gold font-semibold">{audio.platform} Store</span>
                  <span
                ...
                <div className="grid grid-cols-2 gap-2 p-3 rounded-xl text-xs mb-4" style={{ background: 'var(--bg-raised)' }}>
                  <div>
                    ...
                    <span className="font-extrabold text-white text-sm">{formatPrice(audio.current_price)}</span>
                  </div>
                  <div>
                    <span className="block text-[10px]" style={{ color: 'var(--text-muted)' }}>30-Day Median</span>
                    <span className="font-bold text-gray-300 text-sm">{formatPrice(audio.observed_median)}</span>
                  </div>
                </div>
              ...
              <div className="mt-6 pt-4 border-t flex items-center justify-between text-xs font-semibold text-gold" style={{ borderColor: 'var(--sm-border)' }}>
                ...
                <span className="text-[10px] text-gray-400 font-normal">{audio.observation_count} snapshots tracked</span>
              </div>
            </Link>
          );
        })}

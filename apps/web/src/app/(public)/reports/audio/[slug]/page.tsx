...
export async function generateMetadata({ params }: ReportPageProps): Promise<Metadata> {
  ...
  const report = audioData.find((r) => r.slug === slug);
  ...
  return {
    ...
    openGraph: {
      ...
      url: `https://www.shadowmerchant.online/reports/audio/${slug}`,
    },
    alternates: { canonical: `https://www.shadowmerchant.online/reports/audio/${slug}` },
  };
}

export async function generateStaticParams() {
  return audioData.map((r) => ({ slug: r.slug }));
}

export default async function AudioReportDetailPage({ params }: ReportPageProps) {
  const { slug } = await params;
  const report = audioData.find((r) => r.slug === slug);
  if (!report) notFound();
  ...
  return (
    <main className="w-full max-w-4xl mx-auto px-4 py-10 sm:py-14">
      <Link href="/reports/audio" className="text-xs font-semibold text-gold hover:underline mb-6 inline-block">
        ← Back to all Audio & Headphones Decision Reports
      </Link>

      <article className="prose prose-invert max-w-none">
        <div className="flex items-center gap-3 mb-3">
          <span className="text-xs font-semibold uppercase tracking-wider text-gold">
            🎧 Audio Decision Report #{report.id}
          </span>
          <span

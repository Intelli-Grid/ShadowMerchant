...
export async function generateMetadata({ params }: ReportPageProps): Promise<Metadata> {
  ...
  const report = watchData.find((r) => r.slug === slug);
  ...
  return {
    ...
    openGraph: {
      ...
      url: `https://www.shadowmerchant.online/reports/smartwatches/${slug}`,
    },
    alternates: { canonical: `https://www.shadowmerchant.online/reports/smartwatches/${slug}` },
  };
}

export async function generateStaticParams() {
  return watchData.map((r) => ({ slug: r.slug }));
}

export default async function SmartwatchReportDetailPage({ params }: ReportPageProps) {
  const { slug } = await params;
  const report = watchData.find((r) => r.slug === slug);
  if (!report) notFound();
  ...
  return (
    <main className="w-full max-w-4xl mx-auto px-4 py-10 sm:py-14">
      <Link href="/reports/smartwatches" className="text-xs font-semibold text-gold hover:underline mb-6 inline-block">
        ← Back to all Smartwatch Decision Reports
      </Link>

      <article className="prose prose-invert max-w-none">
        <div className="flex items-center gap-3 mb-3">
          <span className="text-xs font-semibold uppercase tracking-wider text-gold">
            ⌚ Smartwatch Decision Report #{report.id}
          </span>
          <span

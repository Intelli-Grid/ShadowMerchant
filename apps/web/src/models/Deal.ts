...
const DealSchema = new Schema({
  ...
  // SEO-FIX-02: Human-readable slug for SEO URLs (/deals/product-name-68pct-off-amazon)
  // Generated by deal_processor.py on creation. Sparse so existing deals don't conflict.
  slug:             { type: String },
...
// SEO-FIX-02: unique slug index — sparse allows null (existing deals without a slug)
DealSchema.index({ slug: 1 }, { unique: true, sparse: true });
...
// PSEO: brand pages — /deals/brand/[brand] queries this index
DealSchema.index({ brand: 1, is_active: 1, deal_score: -1 });
